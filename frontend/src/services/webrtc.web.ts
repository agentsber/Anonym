// Web-specific WebRTC implementation using browser native APIs
...
class WebRTCService {
  ...
  async switchCamera(): Promise<void> {
    ...
    try {
      ...
      if (this.peerConnection) {
        const sender = this.peerConnection.getSenders().find((s: RTCRtpSender) => s.track?.kind === 'video');
        if (sender) {
