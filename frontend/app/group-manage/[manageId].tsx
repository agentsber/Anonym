...
export default function GroupManageScreen() {
  ...
  const loadGroupData = async () => {
    if (!manageId) return;
    try {
      const groupInfo = await groupsApi.getGroupInfo(manageId);
      setGroup(groupInfo);
  ...
  const handleSaveName = async () => {
    if (!manageId || !user || !editName.trim()) return;
    setIsSaving(true);
    try {
      await groupsApi.updateGroup(manageId, { name: editName.trim() }, user.id);
  ...
  const handleAddMember = async (contactId: string) => {
    ...
    try {
      await groupsApi.addMember(manageId, contactId, user.id);
      ...
      [
        { text: 'Отмена', style: 'cancel' },
        {
          ...
          onPress: async () => {
            if (!manageId || !user) return;
            try {
              await groupsApi.removeMember(manageId, memberId, user.id);
              loadGroupData();
            } catch (err: any) {
      ...
      [
        { text: 'Отмена', style: 'cancel' },
        {
          ...
          onPress: async () => {
            if (!manageId || !user) return;
            try {
              await groupsApi.removeMember(manageId, user.id, user.id);
              router.replace('/(tabs)');
            } catch (err: any) {
      ...
      [
        { text: 'Отмена', style: 'cancel' },
        {
          ...
          onPress: async () => {
            if (!manageId || !user) return;
            try {
              await groupsApi.deleteGroup(manageId, user.id);
              router.replace('/(tabs)');
            } catch (err: any) {
